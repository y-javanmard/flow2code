...
import { nodeTypes, palette } from "./nodes";
...
async function generateWithGemini(promptPack: string, language: string) {
  const r = await fetch("/api/generate-code", {
    ...
    body: JSON.stringify({ promptPack, language }),
  });
      ...
      >
        {allPresets.map((p) => (
          <button
            key={p.id}
            onClick={() =>
              setActivePreset(activePresetId === p.id ? null : p.id)
            }
            style={{
              fontSize: 11,
              fontWeight: 800,
              ...
              border:
                activePresetId === p.id
                  ...
                  : "1px solid #d1d5db",
              background:
                activePresetId === p.id ? `${p.color}15` : "white",
...
function Inspector() {
  ...
  return (
    <Panel
      title={`${t.charAt(0).toUpperCase() + t.slice(1)}${d.label ? `: ${d.label}` : ""}`}
      icon="🔍"
      color={typeColor}
    >
      <div
      ...
      >
        <div>
          ...
          <input
            ...
            value={String(d.w ?? "")}
            onChange={(e) =>
              updateNodeData(n.id, { w: Number(e.target.value || 240) })
            }
          />
        </div>
        <div>
          ...
          <input
            ...
            value={String(d.h ?? "")}
            onChange={(e) =>
              updateNodeData(n.id, { h: Number(e.target.value || 90) })
            }
          />
        </div>
      </div>
      ...
      {t === "terminator" && (
        <>
          ...
          <input
            ...
            onChange={(e) => updateNodeData(n.id, { title: e.target.value })}
          />
        </>
      )}
      {t === "process" && (
        <>
          ...
          <textarea
            ...
            value={String(d.stmt ?? "")}
            onChange={(e) => updateNodeData(n.id, { stmt: e.target.value })}
          />
        </>
      )}
      {t === "decision" && (
        <>
          ...
          <input
            style={inputStyle}
            value={String(d.cond ?? "")}
            onChange={(e) => updateNodeData(n.id, { cond: e.target.value })}
          />
        </>
      )}
      {t === "comment" && (
        <>
          ...
          <textarea
            ...
            value={String(d.text ?? "")}
            onChange={(e) => updateNodeData(n.id, { text: e.target.value })}
          />
        </>
      )}
      ...
      {t === "io" && (
        <>
          ...
          <select
            style={inputStyle}
            value={String(d.kind ?? "Input")}
            onChange={(e) => updateNodeData(n.id, { kind: e.target.value })}
          >
            <option value="Input">Input</option>
            <option value="Output">Output</option>
          </select>
          <label style={{ ...labelStyle, marginTop: 10 }}>
            {d.kind === "Output" ? "Expression" : "Variables"}
          </label>
          <input
            style={inputStyle}
            value={String(
              d.kind === "Output" ? (d.expr ?? "") : (d.vars ?? "")
            )}
            onChange={(e) =>
              updateNodeData(
                n.id,
      ...
      {t === "loop" && (
        <>
          ...
          <input
            style={inputStyle}
            value={String(d.spec ?? "")}
            onChange={(e) => updateNodeData(n.id, { spec: e.target.value })}
          />
        </>
      )}
      ...
      {t === "trycatch" && (
        <>
          ...
          <textarea
            ...
            value={String(d.tryBody ?? "")}
            onChange={(e) =>
              updateNodeData(n.id, { tryBody: e.target.value })
            }
          />
          <label style={{ ...labelStyle, marginTop: 10 }}>Catch var</label>
          <input
            style={inputStyle}
            value={String(d.catchVar ?? "")}
            onChange={(e) =>
              updateNodeData(n.id, { catchVar: e.target.value })
            }
          />
          <label style={{ ...labelStyle, marginTop: 10 }}>Catch body</label>
          <textarea
            style={{ ...inputStyle, minHeight: 60, resize: "vertical" }}
            value={String(d.catchBody ?? "")}
            onChange={(e) =>
              updateNodeData(n.id, { catchBody: e.target.value })
            }
          />
        </>
      )}
      ...
      {t === "domain" && (
        <>
          ...
          <input
            style={inputStyle}
            value={String(d.instanceName ?? "")}
            onChange={(e) =>
              updateNodeData(n.id, { instanceName: e.target.value })
            }
          />

          {Array.isArray(d.presetParams) && d.presetParams.length > 0 && (
            <>
              <label style={{ ...labelStyle, marginTop: 12, fontWeight: 900 }}>
                Parameters
              </label>
              {d.presetParams.map((p: any) => (
                <div key={p.name} style={{ marginBottom: 6 }}>
                  ...
                  <input
                    style={inputStyle}
                    value={String(
                      (d.paramValues ?? {})[p.name] ?? p.default
                    )}
                    onChange={(e) =>
                      updateNodeData(n.id, {
                        paramValues: {
          ...
          {d.outputType && (
            <div style={{ marginTop: 10, fontSize: 11 }}>
              <span style={{ fontWeight: 800, color: "#555" }}>
                Output type:{" "}
              </span>
      ...
      {t === "definition" && (
        <div style={{ fontSize: 12, color: "#666", marginTop: 6 }}>
          Edit this definition directly on the canvas node. Use the kind tabs
          (Function/Class/Struct/Enum/Interface) and the "extends" dropdown to
  ...
  return (
    ...
    >
      ...
      >
        ...
        <button style={actionBtn} onClick={onExport}>
          📋 Pack
        </button>
        <button style={actionBtn} onClick={onExportPng}>
          🖼 PNG
        </button>
        <button style={actionBtn} onClick={onSaveProject}>
          💾 Save
        </button>
        <button
          style={{
      ...
      >
        <NodeBtn
          label="Start/End"
          color={palette.terminator.accent}
          onClick={() => addAtCenter("terminator")}
        />
        <NodeBtn
          label="Process"
          color={palette.process.accent}
          onClick={() => addAtCenter("process")}
        />
        <NodeBtn
          label="Decision"
          color={palette.decision.accent}
          onClick={() => addAtCenter("decision")}
        />
        <NodeBtn
          label="I/O"
          color={palette.io.accent}
          onClick={() => addAtCenter("io")}
        />
        <NodeBtn
          label="Loop"
          color={palette.loop.accent}
          onClick={() => addAtCenter("loop")}
        />
        <NodeBtn
          label="Definition"
          color={palette.definition.accent}
          onClick={() => addAtCenter("definition")}
        />
        <NodeBtn
          label="Try/Catch"
          color={palette.trycatch.accent}
          onClick={() => addAtCenter("trycatch")}
        />
        <NodeBtn
          label="Note"
          color={palette.comment.accent}
          onClick={() => addAtCenter("comment")}
        />
      </div>
...
function getTemplatePresetsMap() {
  ...
  const neuralNetTemplates = [
    ...
    },
    {
      ...
      color: "#0891b2",
      params: [],
      desc: "Embedding layer for sequences",
      outputType: "tensor",
      inputTypes: ["tensor"],
    },
    {
    ...
    },
    {
      ...
      color: "#c026d3",
      params: [],
      desc: "Full transformer encoder block",
      outputType: "tensor",
      inputTypes: ["tensor"],
    },
    {
    ...
    },
    {
      ...
      color: "#64748b",
      params: [],
      desc: "Element-wise add (skip connection)",
      outputType: "tensor",
      inputTypes: ["tensor", "tensor"],
    },
    {
      key: "concatenate",
      icon: "🔗",
...
function AppContent() {
  ...
  const handleSelectionChange = React.useCallback(
    (sel: any) => {
      const newId = sel?.nodes?.[0]?.id ?? null;
  ...
  function startDrag(e: React.MouseEvent) {
    ...
    function onMove(ev: MouseEvent) {
      setSidebarW(Math.max(320, Math.min(1000, startW + (startX - ev.clientX))));
    }
    function onUp() {
  ...
  function exportPack() {
    setPack(makePromptPack(nodes as any, edges as any, language));
  }
  ...
  async function exportPng() {
    ...
    const dataUrl = await toPng(canvasRef.current, {
      ...
      filter: (node) =>
        !(node as HTMLElement).classList?.contains("no-export"),
    });
  ...
  async function saveProject() {
    try {
      ...
      const effectivePack = pack.trim()
        ? pack
        : makePromptPack(nodes as any, edges as any, language);
      const pngDataUrl = canvasRef.current
        ? await toPng(canvasRef.current, {
            cacheBust: true,
            backgroundColor: "#f8fafc",
            filter: (n) =>
              !(n as HTMLElement).classList?.contains("no-export"),
          })
        : "";
      const r = await fetch("/api/save-project", {
        ...
        body: JSON.stringify({
          ...
          flow: { nodes, edges },
          promptPack: effectivePack,
          pngDataUrl,
          generated: { language, files: genFiles, notes: genNotes },
        }),
      });
      if (!r.ok) throw new Error(await r.text());
      const out = await r.json();
      setSaveStatus(`Saved: ${out.dir}`);
    } catch (e: any) {
  ...
  async function runGeminiCodegen() {
    try {
      ...
      setGenFiles(out.files ?? []);
      setGenNotes(String(out.notes ?? ""));
    } catch (e: any) {
    ...
    <div style={{ display: "flex", height: "100vh", width: "100vw" }}>
      ...
      >
        ...
        >
          <Toolbar
            onExport={exportPack}
            onGenerate={runGeminiCodegen}
            onExportPng={exportPng}
            onSaveProject={saveProject}
            isGenerating={isGenerating}
            onShowArchBuilder={() => setShowArchBuilder(true)}
          />
          ...
          >
            <div style={{ display: "grid", gap: 10 }}>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                <button style={actionBtn} onClick={exportPng}>
                  🖼 PNG
                </button>
                <button style={actionBtn} onClick={saveProject}>
                  💾 Save
                </button>
              </div>
              {saveStatus && (
